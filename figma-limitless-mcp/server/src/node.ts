...
import type { BridgeResponse, ConnectedFile, SendOpts } from "./types.js";
...
export class Node {
  ...
  /** Supplies the default agent identity journaled with every op. */
  agentSupplier: () => string = () => `client#${process.pid}`;
  ...
  sendWithParams(
    ...
    const agent = opts?.agent ?? this.agentSupplier();
    if (this._role === Role.Leader && this.leader) {
      return this.leader.execute(
        requestType,
        nodeIds,
        params,
        fileKey,
