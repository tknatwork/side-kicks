...
- The linter treats the color and opacity aliases as **alias edges**: they count for tier classification, alias-in-every-mode, component → semantic, one-tier-down, depth/cycles and usage, and a reference that resolves to no local variable is an `alias-target-resolves` error, exactly like a plain alias.
