...
All are detectable via Figma Plugin API 1.130 — `getLocalVariableCollectionsAsync()` (`.modes`, `.defaultModeId`, `.variableIds`) and `getLocalVariablesAsync()` (`.resolvedType`, `.scopes`, `.valuesByMode`, `.variableCollectionId`, `.codeSyntax`, `.hiddenFromPublishing`). Layer identity (primitive/semantic/component) is **inferred structurally**: classify each `valuesByMode` entry as `LITERAL` vs `VARIABLE_ALIAS` (`value.type === "VARIABLE_ALIAS"`; a composed color `{color, opacity}` counts as an alias edge through its aliased color side — its opacity side, a FLOAT, doesn't tier the collection — Figma Update 139), then a collection is *primitive-like* if ~all values are literals, *semantic-like* if ~all values are aliases whose targets resolve into primitive-like collections, *component-like* if ~all aliases target semantic-like collections — cross-checked against collection names. An alias to an imported team-library variable targets a collection the file can't see, so it proves the collection isn't primitive-like but not which of the other two it is.

1. **Primitives are mode-less.** A primitive-layer collection MUST have `modes.length === 1`; flag any primitive collection with more.
2. **Semantic tokens are aliases in every mode.** For every variable in a semantic collection, EVERY `valuesByMode` entry MUST be `{type:"VARIABLE_ALIAS"}` — or a composed color whose color and/or opacity is an alias, which counts as an alias edge. A raw literal in any semantic mode is an `error` (the "raw hex per mode" anti-pattern).
