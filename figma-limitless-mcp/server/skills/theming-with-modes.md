...
All are detectable via Figma Plugin API 1.130 — `getLocalVariableCollectionsAsync()` (`.modes`, `.defaultModeId`, `.variableIds`) and `getLocalVariablesAsync()` (`.resolvedType`, `.scopes`, `.valuesByMode`, `.variableCollectionId`, `.codeSyntax`, `.hiddenFromPublishing`). Layer identity (primitive/semantic/component) is **inferred structurally**: classify each `valuesByMode` entry as `LITERAL` vs `VARIABLE_ALIAS` (`value.type === "VARIABLE_ALIAS"`; a composed color `{color, opacity}` whose color and/or opacity is an alias counts as an alias edge, Figma Update 139), then a collection is *primitive-like* if ~all values are literals, *semantic-like* if ~all values are aliases whose targets resolve into primitive-like collections, *component-like* if ~all aliases target semantic-like collections — cross-checked against collection names.
