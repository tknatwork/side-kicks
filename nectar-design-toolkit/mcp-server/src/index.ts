...
function figmaRequest(endpoint: string): Promise<unknown> {
...
function bridgeCommand(command: string, payload: Record<string, unknown> = {}): Promise<unknown> {
  return new Promise((resolve, reject) => {
...
function checkBridgeStatus(): Promise<{ connected: boolean; fileInfo: { name: string; id: string; currentPage: string } | null }> {
  return new Promise((resolve, reject) => {
    const options = {
      hostname: BRIDGE_HOST,
      port: BRIDGE_PORT,
...
function tokensToCss(tokens: Record<string, unknown>): string {
  ...
  function processToken(obj: Record<string, unknown>, prefix: string = "") {
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (typeof value === "object" && value !== null && "value" in value) {
        ...
        processToken(value as Record<string, unknown>, varName);
      }
    }
  }
...
server.tool(
  "fetch_figma_tokens",
  "Fetch design tokens (colors, typography, spacing) from the connected Figma file. Returns raw token data without saving.",
  {},
  async () => {
    if (!FILE_ID) {
      return {
    ...
    try {
      const data = await figmaRequest(`/v1/files/${FILE_ID}/variables/local`) as {
        meta?: {
      ...
      const variables = data.meta.variables as Record<string, {
        name: string;
        resolvedType: string;
      ...
      const variableCollections = data.meta.variableCollections as Record<string, {
        name: string;
      }>;
      ...
      Object.values(variables).forEach((variable) => {
        ...
        let value = variable.valuesByMode[modeId] as { type?: string; r?: number; g?: number; b?: number; a?: number };
        ...
        let current = tokens[collectionName] as Record<string, unknown>;
        ...
        if (variable.resolvedType === "COLOR" && typeof value === "object" && "r" in value) {
          value = rgbaToHex(value as { r: number; g: number; b: number; a: number }) as unknown as typeof value;
        }
...
server.tool(
  "sync_figma_tokens",
  "Fetch tokens from Figma and sync them to the project. Updates both tokens.json and variables.css files.",
  {},
  async () => {
    if (!FILE_ID) {
      return {
...
server.tool(
  "get_figma_file_info",
  "Get basic information about the connected Figma file including name, last modified date, and page structure.",
  {},
  async () => {
    if (!FILE_ID) {
      return {
    ...
    try {
      const data = await figmaRequest(`/v1/files/${FILE_ID}?depth=1`) as {
        name: string;
        lastModified: string;
        version: string;
        document: {
...
server.tool(
  ...
  "List all published components in the Figma file. Useful for understanding the design system structure.",
  {},
  async () => {
    if (!FILE_ID) {
      return {
    ...
    try {
      const data = await figmaRequest(`/v1/files/${FILE_ID}/components`) as {
        meta?: {
          components?: Array<{
            name: string;
            description: string;
...
server.tool(
  "read_project_tokens",
  "Read the current design tokens stored in the project (tokens.json). Useful for comparing with Figma.",
  {},
  async () => {
    try {
...
server.tool(
  "get_figma_styles",
  "Get all styles (colors, text, effects, grids) defined in the Figma file.",
  {},
  async () => {
    if (!FILE_ID) {
      return {
    ...
    try {
      const data = await figmaRequest(`/v1/files/${FILE_ID}/styles`) as {
        meta?: {
          styles?: Array<{
            name: string;
            style_type: string;
      ...
      const stylesByType: Record<string, Array<{ name: string; description: string }>> = {};

      data.meta.styles.forEach((style) => {
        const type = style.style_type;
        if (!stylesByType[type]) stylesByType[type] = [];
...
server.resource(
  "design-context",
  "Current design system context including tokens and configuration",
  async () => {
...
server.tool(
  "check_bridge_status",
  "Check if the Bridge Server is running and if the Figma Plugin is connected. Required before using any write operations.",
  {},
  async () => {
    try {
...
server.tool(
  ...
  "Create a new variable collection in Figma with optional modes (e.g., Light/Dark). Requires Bridge Server and Plugin.",
  {
  ...
  async ({ name, modes }) => {
    try {
      const result = await bridgeCommand("create_variable_collection", { name, modes }) as {
        id: string;
        name: string;
...
server.tool(
  "create_variable",
  "Create a new variable in a Figma collection. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "set_variable_value",
  "Set the value of a variable for a specific mode. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "create_color_style",
  "Create a new color style in Figma. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "create_text_style",
  "Create a new text style in Figma. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "create_effect_style",
  "Create a new effect style (shadow, blur) in Figma. Requires Bridge Server and Plugin.",
  {
  ...
  async ({ name, effects, description }) => {
    try {
      const processedEffects = effects.map(effect => {
        const processed: Record<string, unknown> = { type: effect.type };
        
        if (effect.color) {
          const rgba = hexToRgba(effect.color.hex);
          processed.color = { r: rgba.r, g: rgba.g, b: rgba.b, a: effect.color.alpha ?? rgba.a };
        }
        if (effect.offset) processed.offset = effect.offset;
        if (effect.radius !== undefined) processed.radius = effect.radius;
        if (effect.spread !== undefined) processed.spread = effect.spread;
...
server.tool(
  "create_figma_page",
  "Create a new page in the Figma file. Requires Bridge Server and Plugin.",
  {
    name: z.string().describe("Page name"),
  },
  async ({ name }) => {
    try {
      const result = await bridgeCommand("create_page", { name }) as { id: string; name: string };
...
server.tool(
  "get_figma_pages_live",
  "Get current pages from the open Figma file via Plugin. Requires Bridge Server and Plugin.",
  {},
  async () => {
    try {
      const result = await bridgeCommand("get_pages", {}) as Array<{ id: string; name: string }>;
...
server.tool(
  "get_variable_collections_live",
  "Get current variable collections from the open Figma file via Plugin. Requires Bridge Server and Plugin.",
  {},
  async () => {
    try {
      const result = await bridgeCommand("get_variable_collections", {}) as Array<{
        id: string;
        name: string;
...
server.tool(
  "get_variables_live",
  "Get current variables from the open Figma file via Plugin. Requires Bridge Server and Plugin.",
  {
    collectionId: z.string().optional().describe("Filter by collection ID"),
  },
  async ({ collectionId }) => {
    try {
      const result = await bridgeCommand("get_variables", { collectionId }) as Array<{
        id: string;
        name: string;
        resolvedType: string;
        collectionId: string;
...
server.tool(
  "create_frame",
  "Create a new frame in the current Figma page. Requires Bridge Server and Plugin.",
  {
  ...
  async ({ name, width, height, x, y, fillHex, layoutMode, itemSpacing, padding }) => {
    try {
      ...
      if (itemSpacing !== undefined) payload.itemSpacing = itemSpacing;
      if (padding) payload.padding = padding;
      ...
      const result = await bridgeCommand("create_frame", payload) as { id: string; name: string };
...
server.tool(
  "set_current_page",
  "Navigate to a specific page in the Figma file. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "create_text",
  "Create a text node in the current Figma page. Requires Bridge Server and Plugin.",
  {
  ...
  async ({ text, x, y, fontSize, fontFamily, fontWeight, fillHex, parentId }) => {
    try {
      ...
      if (fontWeight) payload.fontWeight = fontWeight;
      ...
      const result = await bridgeCommand("create_text", payload) as { id: string };
      return {
...
server.tool(
  "update_text",
  "Update text content of an existing text node. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "delete_node",
  "Delete a node from the Figma file. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "delete_variable",
  "Delete a variable from Figma. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "set_variable_scopes",
  "Set the scopes of a variable (which properties it can be applied to). Requires Bridge Server and Plugin.",
  {
    variableId: z.string().describe("ID of the variable"),
...
server.tool(
  ...
  "Bind a variable to a node property in Figma. Requires Bridge Server and Plugin.",
  {
...
server.tool(
  "get_local_styles_live",
  "Get all local styles (paint, text, effect) from the open Figma file. Requires Bridge Server and Plugin.",
  {},
  async () => {
    try {
      const result = await bridgeCommand("get_local_styles", {}) as Array<{
        id: string;
        name: string;
        type: string;
      }>;
...
server.tool(
  "run_build_ds",
  "Run the automated Figma Design System builder (build-figma-ds.js). Creates all variables, styles, and pages from Portfolio token files. Requires Bridge Server and Plugin.",
  {
  ...
  async ({ step, dryRun }) => {
    try {
      ...
      const args: string[] = [];
      if (step && step !== "all") { args.push("--step", step); }
...
server.prompt(
  "sync-design-tokens",
  "A guided workflow to sync design tokens from Figma to the project",
  async () => {
    return {
      messages: [
        {
...
server.prompt(
  "build-design-system",
  "A guided workflow to build the Nectar Core design system in Figma using the Plugin",
  async () => {
    return {
      messages: [
        {
