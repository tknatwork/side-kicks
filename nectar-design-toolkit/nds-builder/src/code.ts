...
// ============================================================================
// ============================================================================
// NODE EXTRACTION INTERFACES
// Interfaces for extracting Figma document structure
// ============================================================================
...
interface PageInfo {
  ...
  name: string;
  childCount: number;
  frames: { id: string; name: string; type: string; width: number; height: number }[];
}
...
interface ExtractionResult {
  ...
  fileKey?: string;
  pages?: PageInfo[];
  selection?: ExtractedNode[];
  nodeTree?: ExtractedNode;
}

// ============================================================================
// BRAND COLOR PRIMITIVES - NDS Color Palette
// Full color scales for Pink, Purple, Green, Orange, Red, Yellow, Neutral
// ============================================================================
...
// ============================================================================
// ANT DESIGN TO NDS COLOR MAPPING
// Maps Ant Design color palette names to NDS brand colors
// Ant uses 1-10 scale (1=lightest), NDS uses 100-900 (100=lightest)
// ============================================================================

interface ColorMapping {
  antName: string;  // e.g., 'gold'
  ndsPrefix: string;  // e.g., 'orange'
  scales: { ant: number; nds: string }[];  // mapping ant scale to nds scale
}

// Colors to UPDATE (existing Ant palettes → NDS values)
const ANT_TO_NDS_MAPPING: ColorMapping[] = [
  {
    antName: 'gold',
    ndsPrefix: 'orange',
    scales: [
...
async function inspectGridStyles(): Promise<void> {
...
async function buildGridStyles(): Promise<void> {
  ...
  try {
    ...
    for (var i = 0; i < GRID_STYLES.length; i++) {
      ...
      if (gridDef.pattern === 'GRID') {
        ...
        style.layoutGrids = [{
          pattern: 'COLUMNS',
          alignment: (gridDef.alignment || 'STRETCH') as 'MIN' | 'MAX' | 'STRETCH' | 'CENTER',
...
async function buildTextStyles(): Promise<void> {
...
async function buildEffectStyles(): Promise<void> {
  ...
  for (const style of EFFECT_STYLES) {
    ...
    effectStyle.effects = style.effects as Effect[];
    created++;
    log(`Created: ${style.name}`);
  }
...
async function buildBrandColors(): Promise<void> {
...
async function inspectVariableCollections(): Promise<void> {
...
async function updateColorsCollection(): Promise<void> {
  log('🎨 Updating colors collection with NDS brand colors...');
  
  try {
    // Find the "colors" collection
    const collections = figma.variables.getLocalVariableCollections();
    let colorsCollection: VariableCollection | null = null;
    ...
    for (const scaleName of NEW_COLOR_SCALES) {
      ...
      const ndsToAntScale: { [key: string]: number } = {
...
async function cleanAll(): Promise<void> {
  ...
  for (const s of textStyles) {
    try { s.remove(); textCount++; } catch (e) { /* ignore */ }
  }
  ...
  for (const s of effectStyles) {
    try { s.remove(); effectCount++; } catch (e) { /* ignore */ }
  }
  ...
  for (const s of gridStyles) {
    try { s.remove(); gridCount++; } catch (e) { /* ignore */ }
  }
  ...
  if (pages.length > 1) {
    ...
    for (let i = pages.length - 1; i >= 1; i--) {
      try { pages[i].remove(); pageCount++; } catch (e) { /* ignore */ }
    }
  }
...
async function cleanTextStylesOnly(): Promise<void> {
  log('🧹 Cleaning text styles...');
  const textStyles = await figma.getLocalTextStylesAsync();
  let textCount = 0;
  for (const s of textStyles) {
...
async function cleanEffectStylesOnly(): Promise<void> {
  log('🧹 Cleaning effect styles...');
  const effectStyles = await figma.getLocalEffectStylesAsync();
  let effectCount = 0;
  for (const s of effectStyles) {
...
async function cleanPagesOnly(): Promise<void> {
  log('🧹 Cleaning pages...');
  const pages = figma.root.children;
  let pageCount = 0;
  if (pages.length > 1) {
...
async function cleanGridStylesOnly(): Promise<void> {
  log('🧹 Cleaning grid styles...');
  const gridStyles = await figma.getLocalGridStylesAsync();
  let gridCount = 0;
  for (const s of gridStyles) {
 ...
 */
function paintToString(paints: readonly Paint[] | typeof figma.mixed): string {
 ...
 */
function effectsToString(effects: readonly Effect[]): string {
 ...
 */
function extractNodeProperties(node: SceneNode, depth: number = 0, maxDepth: number = 10): ExtractedNode {
  ...
  if ('x' in node) extracted.x = Math.round(node.x);
  if ('y' in node) extracted.y = Math.round(node.y);
  if ('width' in node) extracted.width = Math.round(node.width);
  if ('height' in node) extracted.height = Math.round(node.height);

  // Visual properties
  if ('fills' in node) extracted.fills = paintToString(node.fills);
  if ('strokes' in node) extracted.strokes = paintToString(node.strokes);
  if ('effects' in node) extracted.effects = effectsToString(node.effects);
  ...
  if (node.type === 'TEXT') {
    ...
    if (textNode.fontSize !== figma.mixed) extracted.fontSize = textNode.fontSize;
    if (textNode.fontName !== figma.mixed) extracted.fontName = `${textNode.fontName.family} ${textNode.fontName.style}`;
 ...
 */
async function extractPageStructure(): Promise<void> {
  log('📄 Extracting page structure...');
  
  const pages: PageInfo[] = [];
  
  for (const page of figma.root.children) {
    const frames: { id: string; name: string; type: string; width: number; height: number }[] = [];
...
async function extractSelection(): Promise<void> {
  ...
  const extractedNodes: ExtractedNode[] = selection.map(node => extractNodeProperties(node, 0, 3));
  ...
  for (const node of extractedNodes) {
    ...
    if (node.childCount) log(`   └─ ${node.childCount} children`);
  }
 ...
 */
async function extractNodeTree(depth: number = 5): Promise<void> {
  ...
  // Count total nodes
  function countNodes(n: ExtractedNode): number {
    let count = 1;
    if (n.children) {
...
figma.ui.onmessage = async (msg: { type: string; depth?: number; data?: any; filename?: string }) => {
  switch (msg.type) {
    // Build commands
    case 'build_all': await buildAll(); break;
    case 'build_pages': await buildPages(); break;
    case 'build_text_styles': await buildTextStyles(); break;
    case 'build_effect_styles': await buildEffectStyles(); break;
    case 'build_grid_styles': await buildGridStyles(); break;
    case 'build_brand_colors': await buildBrandColors(); break;
    case 'update_colors_collection': await updateColorsCollection(); break;
    
    // Inspect commands
    case 'inspect_grid_styles': await inspectGridStyles(); break;
    case 'inspect_variable_collections': await inspectVariableCollections(); break;
    
    // Extraction commands
    case 'extract_page_structure': await extractPageStructure(); break;
    case 'extract_selection': await extractSelection(); break;
    case 'extract_node_tree': await extractNodeTree(msg.depth || 5); break;
    case 'export_json': await exportToJson(msg.data, msg.filename || 'figma-export.json'); break;
    
    // Clean commands
    case 'clean_all': await cleanAll(); break;
    case 'clean_text_styles': await cleanTextStylesOnly(); break;
    case 'clean_effect_styles': await cleanEffectStylesOnly(); break;
    case 'clean_pages': await cleanPagesOnly(); break;
    case 'clean_grid_styles': await cleanGridStylesOnly(); break;
